...
The Blazor app uses the **Interactive WebAssembly** render mode applied **globally with prerendering disabled** — `<Routes @rendermode="@(new InteractiveWebAssemblyRenderMode(prerender: false))" />` in [App.razor](App/VisiLog/VisiLog.Web.Blazor.WASM/VisiLog.Web.Blazor.WASM/Components/App.razor) — so every routed component runs in the browser only. This drives a strict project split:
...
**Why `prerender: false` matters**: server-side DI only registers the server layers (Logic, Service.WebAPI, Data.SQL); the HTTP-client abstractions (`ILogSourceClient`, `ILogMessageClient`, etc. from `VisiLog.Abstraction.Service.WebAPI`) are registered only in the WASM container. If prerendering were enabled, the server would try to instantiate page components on the initial request, hit their `[Inject]` properties for those clients, and throw *"No registered service of type 'ILogSourceClient'."* Keep prerendering off at the root — setting `@rendermode` on a child page **cannot** override the parent's effective rendermode (Blazor inherits down the tree), so the only place this flag is meaningful is on `<Routes>` in `App.razor`. Do not add `@rendermode` directives on individual pages; they are silently ignored.

**Why `Routes.razor` must live in the Client project**: `App.razor` (server-side) renders `<Routes @rendermode="@(new InteractiveWebAssemblyRenderMode(prerender: false))" />`. The WASM runtime then needs to instantiate the `Routes` type in the browser, which requires its assembly to be downloaded — only the `.Client` assembly is. Putting `Routes.razor` (or any routed component) in the host project produces a runtime error: *"Root component type ... could not be found in the assembly 'VisiLog.Web.Blazor.WASM'."*
...
- Repository bindings (`services.AddScoped<ILogMessageRepository, LogMessageRepository>()`) and the `ILogSourceResolver` registration belong in the `LoadManualRegistration` phase of [VisiLog.Data.SQL/Loader.cs](App/VisiLog/VisiLog.Data.SQL/Loader.cs), not in `Program.cs`.
...
- Per-source connection info is resolved by name via [ILogSourceResolver.Resolve(logSourceName)](App/VisiLog/VisiLog.Data.SQL/Sources/LogSourceResolver.cs), which looks up the matching `LogSource` in `IOptions<List<LogSource>>` (bound from the `"LogSources"` array in `appsettings.json`) and returns a `(string ConnectionString, string TableName)` tuple. Repositories take a `logSourceName` parameter on every method, call the resolver, and construct the `SqlConnection` themselves. Do not bypass the resolver by reading `IConfiguration.GetConnectionString` directly in repositories, and do not add a parallel lookup path — there should be exactly one resolution surface.
